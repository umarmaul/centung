...
// Include your custom headers AFTER standard/library headers
#include "config.h"   // Contains pin definitions (I2C_SDA_PIN, I2C_SCL_PIN)
#include "library.h"  // Contains constants if any remaining

// --- Configuration & Credentials ---
// Firebase config (Replace with your actual credentials)
#define FIREBASE_HOST "YOUR_PROJECT_ID.firebaseio.com"  // Or your specific RTDB URL
#define FIREBASE_AUTH "YOUR_DATABASE_SECRET"            // Or use other auth methods
#define FIREBASE_PROJECT_ID "YOUR_PROJECT_ID"
#define WIFI_SSID "Gedung MRPQ"   // Optional: Define if not using WiFiManager Portal
#define WIFI_PASSWORD "umar1234"  // Optional: Define if not using WiFiManager Portal

// --- MPU6050 Kalman Filter Defines & Address ---
...
const int CalibrationCount = 2000;      // Number of readings for calibration
const float AccelScaleFactor = 4096.0;  // LSB/g for +/- 8g range (Check datasheet if different range used)
const float GyroScaleFactor = 65.5;     // LSB/deg/s for +/- 500 deg/s range (Check datasheet if different range used)
const float KalmanDt = 0.004;           // Kalman filter time step in seconds (Matches 4ms loop time = 250Hz)
// Kalman tuning parameters (adjust as needed)
const float KalmanQAngle = 0.001 * 0.001;  // Process noise variance for angle (adjust based on gyro drift)
const float KalmanQBias = 0.003 * 0.003;   // Process noise variance for bias (adjust based on bias stability)
const float KalmanRMeasure = 0.03 * 0.03;  // Measurement noise variance (adjust based on accelerometer noise)

// --- Global Objects ---
// Hardware
HX711_ADC LoadCell(PIN_HX711_DATA, PIN_HX711_CLOCK);
// MPU6050 mpu; // REMOVED - Using direct Wire access now
...
// Buttons
OneButton button1(PUSH_BUTTON_1, true, true);  // Enable pullup, active low
...
// --- Kalman Filter & MPU6050 Global Variables ---
// Raw Sensor Data (temporary, within task preferably)
// float RateRoll, RatePitch, RateYaw; // Gyro rates in deg/s
// float AccX, AccY, AccZ;         // Accel values in g

// Calculated Angles (from Accel)
float AngleRollAcc, AnglePitchAcc;

// Gyro Bias Values
float GyroBiasRoll = 0, GyroBiasPitch = 0, GyroBiasYaw = 0;

// Kalman filter state variables (Roll)
float KalmanAngleRoll = 0;                   // The calculated angle
float KalmanBiasRoll = 0;                    // The calculated gyro bias
float KalmanPRoll[2][2] = {{1, 0}, {0, 1}};  // Error covariance matrix

// Kalman filter state variables (Pitch)
float KalmanAnglePitch = 0;
float KalmanBiasPitch = 0;
float KalmanPPitch[2][2] = {{1, 0}, {0, 1}};

// Smoothing Variables
const int smoothingWindow = 10;  // Keep smoothing? Optional.
float rollWindow[smoothingWindow];
float pitchWindow[smoothingWindow];
int smoothingIndex = 0;

// --- Shared Variables (Protected by Mutexes) ---
// Sensor Data (Protected by xSensorDataMutex)
float currentWeight_kg = 0.0;
float roll = 0.0, pitch = 0.0;  // Output: Smoothed Kalman angles
// float yaw = 0.0; // Yaw calculation is not included in this Kalman filter
float temperature = 0.0;  // Read separately if needed

// State Variables (Protected by xStateMutex)
int currentState = 0;
double KNasiSekarang = 0.0, KNasi_profile1 = 0.0, KNasi_profile2 = 0.0, KNasi_profile3 = 0.0, KNasi_profile4 = 0.0;
double max_nasi = 0.0;
...
boolean needsTare = true;

// Other Global Vars
String devId;
int ulang = 0, cek_berat = 0;
int timeout = 30;
